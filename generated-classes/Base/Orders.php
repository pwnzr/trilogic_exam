...
abstract class Orders implements ActiveRecordInterface
{
     ...
     * @var        ChildAddresses
     */
    protected $aAddresses;

    /**
     * @var        ChildRestaurants
     */
    protected $aRestaurants;
    ...
    public function setAddressId($v)
    {
        ...
        if ($this->address_id !== $v) {
            $this->address_id = $v;
            $this->modifiedColumns[OrdersTableMap::COL_ADDRESS_ID] = true;
        }

        if ($this->aAddresses !== null && $this->aAddresses->getId() !== $v) {
    ...
    public function setRestaurantId($v)
    {
        ...
        if ($this->restaurant_id !== $v) {
            ...
            $this->modifiedColumns[OrdersTableMap::COL_RESTAURANT_ID] = true;
        }

        if ($this->aRestaurants !== null && $this->aRestaurants->getId() !== $v) {
            $this->aRestaurants = null;
        }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[OrdersTableMap::COL_VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : OrdersTableMap::translateFieldName('AddressId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->address_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : OrdersTableMap::translateFieldName('RestaurantId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->restaurant_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : OrdersTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            $this->value = (null !== $col) ? (double) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : OrdersTableMap::translateFieldName('Date', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aAddresses !== null && $this->address_id !== $this->aAddresses->getId()) {
            $this->aAddresses = null;
        }
        if ($this->aRestaurants !== null && $this->restaurant_id !== $this->aRestaurants->getId()) {
            $this->aRestaurants = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aAddresses !== null) {
                if ($this->aAddresses->isModified() || $this->aAddresses->isNew()) {
                    $affectedRows += $this->aAddresses->save($con);
                }
                $this->setAddresses($this->aAddresses);
            }

            if ($this->aRestaurants !== null) {
                if ($this->aRestaurants->isModified() || $this->aRestaurants->isNew()) {
                    $affectedRows += $this->aRestaurants->save($con);
                }
                $this->setRestaurants($this->aRestaurants);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(OrdersTableMap::COL_ADDRESS_ID)) {
            $modifiedColumns[':p' . $index++]  = 'address_id';
        }
        if ($this->isColumnModified(OrdersTableMap::COL_RESTAURANT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'restaurant_id';
        }
        if ($this->isColumnModified(OrdersTableMap::COL_VALUE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'address_id':
                        ...
                        break;
                    case 'restaurant_id':
                        $stmt->bindValue($identifier, $this->restaurant_id, PDO::PARAM_INT);
                        break;
                    case 'value':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->aAddresses) {
                ...
                $result[$key] = $this->aAddresses->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
            if (null !== $this->aRestaurants) {
                ...
                $result[$key] = $this->aRestaurants->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setAddressId($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setRestaurantId($arr[$keys[3]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrdersTableMap::COL_ADDRESS_ID)) {
            $criteria->add(OrdersTableMap::COL_ADDRESS_ID, $this->address_id);
        }
        if ($this->isColumnModified(OrdersTableMap::COL_RESTAURANT_ID)) {
            $criteria->add(OrdersTableMap::COL_RESTAURANT_ID, $this->restaurant_id);
        }
        if ($this->isColumnModified(OrdersTableMap::COL_VALUE)) {
            $criteria->add(OrdersTableMap::COL_VALUE, $this->value);
        }
        if ($this->isColumnModified(OrdersTableMap::COL_DATE)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setRestaurantId($this->getRestaurantId());
    ...
    /**
     * Declares an association between this object and a ChildAddresses object.
     *
     * @param  ChildAddresses $v
     * @return $this|\Orders The current object (for fluent API support)
     * @throws PropelException
     */
    public function setAddresses(ChildAddresses $v = null)
    {
        if ($v === null) {
        ...
        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildAddresses object, it will not be re-added.
        if ($v !== null) {
            $v->addOrders($this);
        }
    ...
    /**
     * Get the associated ChildAddresses object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildAddresses The associated ChildAddresses object.
     * @throws PropelException
     */
    public function getAddresses(ConnectionInterface $con = null)
    {
        if ($this->aAddresses === null && ($this->address_id != 0)) {
            $this->aAddresses = ChildAddressesQuery::create()->findPk($this->address_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aAddresses->addOrderss($this);
             */
        }

        return $this->aAddresses;
    }

    /**
     * Declares an association between this object and a ChildRestaurants object.
     *
     * @param  ChildRestaurants $v
     * @return $this|\Orders The current object (for fluent API support)
     * @throws PropelException
     */
    public function setRestaurants(ChildRestaurants $v = null)
    {
        if ($v === null) {
        ...
        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildRestaurants object, it will not be re-added.
        if ($v !== null) {
            $v->addOrders($this);
        }
    ...
    /**
     * Get the associated ChildRestaurants object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildRestaurants The associated ChildRestaurants object.
     * @throws PropelException
     */
    public function getRestaurants(ConnectionInterface $con = null)
    {
        if ($this->aRestaurants === null && ($this->restaurant_id != 0)) {
            $this->aRestaurants = ChildRestaurantsQuery::create()->findPk($this->restaurant_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aRestaurants->addOrderss($this);
             */
        }

        return $this->aRestaurants;
    }
    ...
    public function clear()
    {
        ...
        if (null !== $this->aAddresses) {
            $this->aAddresses->removeOrders($this);
        }
        if (null !== $this->aRestaurants) {
            $this->aRestaurants->removeOrders($this);
        }
        $this->id = null;
        $this->user_id = null;
